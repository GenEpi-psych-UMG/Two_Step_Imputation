...
for (i in unique(all_data$chr)) {
  ...
  if (nrow(input) == 1) {
    ...
    if (nrow(input) == 2) {
      ...
      write.table(Keep_list, paste("Keep_list_main_chr", Keep_list$chr[1], "_with_reference.txt", sep = ""), row.names = F, col.names = F, quote = F, sep = "\t")
      write.table(Keep_list[,1:2], paste("Keep_list_main_chr", Keep_list$chr[1], ".txt", sep = ""), row.names = F, col.names = F, quote = F, sep = "\t")
      ...
      for (ref in unique_references) {
        ...
        write.table(subset_df[,1:2], paste("Keep_list_main_chr", Keep_list$chr[1],"_", ref ,".txt", sep = ""), row.names = F, col.names = F, quote = F, sep = "\t")
